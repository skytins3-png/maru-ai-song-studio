...
function createScore(){
 if(!$('#lyrics').value){toast('먼저 노래 설계를 만들어 주세요');return}const p=activeProfile(),key=p.key.split(' ')[0],minor=p.key.includes('Minor'),root=KEY_ROOTS[key]||67,scale=minor?[0,2,3,5,7,8,10]:[0,2,4,5,7,9,11],six=p.meter==='6/8',perBar=six?6:4,bars=8,patterns=six?[0,2,4,3,2,1,0,2,3,4,5,4,2,4,6,5,4,2,1,3,5,4,3,2]:[0,2,4,3,1,3,5,4,2,4,6,5,3,2,1,0];scoreNotes=[];
 for(let i=0;i<bars*perBar;i++){const raw=patterns[i%patterns.length],oct=raw>=7?1:0,degree=raw%7,midi=root+scale[degree]+oct*12;scoreNotes.push({midi,degree:degree+oct*7,bar:Math.floor(i/perBar),beat:i%perBar})}
 const chords=KEY_CHORDS[key]||KEY_CHORDS.G,chorus=(LYRICS[profileKey(p)]||LYRICS.generic).chorus;let body='';for(let system=0;system<2;system++){const top=55+system*170,left=70,right=900;for(let l=0;l<5;l++)body+=`<line x1="${left}" y1="${top+l*12}" x2="${right}" y2="${top+l*12}" stroke="#222"/>`;body+=`<text x="25" y="${top+42}" font-size="52">𝄞</text><text x="58" y="${top+22}" font-size="18">${six?'6':'4'}</text><text x="58" y="${top+42}" font-size="18">${six?'8':'4'}</text>`;for(let b=0;b<4;b++){const bar=system*4+b,x0=75+b*207.5;body+=`<line x1="${x0}" y1="${top}" x2="${x0}" y2="${top+48}" stroke="#222"/><text x="${x0+8}" y="${top-13}" font-size="17" font-weight="700">${esc(chords[bar%4])}</text>`;const notes=scoreNotes.filter(n=>n.bar===bar);notes.forEach((n,j)=>{const x=x0+22+j*(175/notes.length),y=top+42-n.degree*6;body+=`<g class="score-note" data-note="${bar*perBar+j}"><ellipse cx="${x}" cy="${y}" rx="7" ry="5" transform="rotate(-18 ${x} ${y})" fill="#111"/><line x1="${x+6}" y1="${y}" x2="${x+6}" y2="${y-30}" stroke="#111" stroke-width="2"/></g>`});body+=`<text x="${x0+8}" y="${top+78}" font-size="13">${esc(chorus[bar%chorus.length])}</text>`}body+=`<line x1="900" y1="${top}" x2="900" y2="${top+48}" stroke="#222" stroke-width="2"/>`}
 const title=esc($('#title').value||'MARU SONG');$('#scoreSheet').innerHTML=`<svg id="scoreSvg" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 930 390" role="img" aria-label="${title} 멜로디 악보"><rect width="930" height="390" fill="white"/><text x="465" y="28" text-anchor="middle" font-size="22" font-weight="800">${title} · Chorus Melody</text>${body}<text x="465" y="382" text-anchor="middle" font-size="12">MARU WORLD MUSIC MAKER V0.4 · 자동 생성 초안</text></svg>`;$('#scoreMeta').textContent=`${p.key} · ${p.meter}박자 · ${p.bpm} BPM · 후렴 8마디 · 코드 ${chords.join('–')}`;toast('후렴 멜로디 오선악보를 만들었습니다')
}
