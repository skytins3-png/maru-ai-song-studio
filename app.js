...
function barRhythm(mode,total,bar,section,instrumental){if(instrumental)return total===4?[2,2]:[2,1];if(mode==='whole')return[total];if(mode==='half')return total===4?[2,2]:[2,1];if(mode==='quarter')return Array(total).fill(1);if(mode==='eighth')return Array(total*2).fill(.5);const mixed=total===4?[[1,.5,.5,1,1],[.5,.5,1,2],[1,1,.5,.5,1],[2,.5,.5,1]]:[[1,.5,.5,1],[.5,.5,1,1],[1,1,1]];if(mode==='mixed')return mixed[bar%mixed.length];if(/Chorus|Hook/.test(section))return mixed[bar%mixed.length];if(/Bridge|Verse/.test(section))return bar%3===0?mixed[bar%mixed.length]:Array(total).fill(1);return total===4?[2,1,1]:[1,1,1]}
...
function createScore(){
 ...
 $('#lyrics').value.split('\n').forEach(raw=>{const line=raw.trim();if(!line)return;if(/^\[.+\]$/.test(line)){section=line.slice(1,-1);return}entries.push({section,text:line.replace(/^\(|\)$/g,''),instrumental:/^\(.+\)$/.test(line)})});const bars=entries.length,systems=Math.ceil(bars/4),height=65+systems*155+30,rhythmMode=$('#rhythmMode').value,dynamicChoice=$('#dynamics').value;scoreNotes=[];let noteIndex=0;
 for(let bar=0;bar<bars;bar++){const durations=barRhythm(rhythmMode,total,bar,entries[bar].section,entries[bar].instrumental);durations.forEach((duration,beat)=>{const raw=patterns[(noteIndex+bar)%patterns.length],oct=raw>=7?1:0,degree=raw%7,midi=root+scale[degree]+oct*12;scoreNotes.push({midi,degree:degree+oct*7,bar,beat,duration,dynamic:dynamicMark(entries[bar].section,dynamicChoice)});noteIndex++})}
 ...
 const title=esc($('#title').value||'MARU SONG'),modeLabel=$('#scoreMode').value==='manual'?'수동':'자동';$('#scoreSheet').innerHTML=`<svg id="scoreSvg" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 930 ${height}" role="img" aria-label="${title} 전체 멜로디 악보"><rect width="930" height="${height}" fill="white"/><text x="465" y="28" text-anchor="middle" font-size="22" font-weight="800">${title} · Full Song Melody</text>${body}<text x="465" y="${height-8}" text-anchor="middle" font-size="12">MARU WORLD MUSIC MAKER V0.6 · ${modeLabel} 전체 곡 악보</text></svg>`;$('#scoreMeta').textContent=`${modeLabel} · ${p.key} · ${p.meter}박자 · ${p.bpm} BPM · 전체 ${bars}마디 · 음표 ${scoreNotes.length}개 · 셈여림 ${dynamicChoice}`;toast(`${modeLabel} 설정으로 전체 곡 악보를 만들었습니다`)
}
function playScore(){if(!scoreNotes.length){createScore();if(!scoreNotes.length)return}stop();const p=scoreSettings(),quarter=60/p.bpm;audio().resume();let elapsed=0;scoreNotes.forEach((n,i)=>{const start=elapsed*quarter,dur=n.duration*quarter,vol=DYNAMIC_VOLUME[n.dynamic]||.07;tone('piano',440*Math.pow(2,(n.midi-69)/12),start,dur*.88,vol);timers.push(setTimeout(()=>{$$('.score-note').forEach(x=>x.classList.remove('active'));$(`[data-note="${i}"]`)?.classList.add('active')},start*1000));elapsed+=n.duration});timers.push(setTimeout(()=>{$$('.score-note').forEach(x=>x.classList.remove('active'))},elapsed*quarter*1000));toast('전주부터 아웃트로까지 전체 곡을 재생합니다')}
...
function updateScoreMode(){const manual=$('#scoreMode').value==='manual';['manualKey','manualMeter','manualBpm'].forEach(id=>$('#'+id).disabled=!manual);if(manual){const p=activeProfile();$('#manualKey').value=p.key;$('#manualMeter').value=p.meter;$('#manualBpm').value=p.bpm}if($('#scoreSvg'))createScore()}
...
async function copySuno(){if(!$('#lyrics').value)return toast('먼저 노래 설계를 만들어 주세요');const text=`[Title]\n${$('#title').value||'제목 없음'}\n\n[Lyrics]\n${$('#lyrics').value}\n\n[Style of Music]\n${$('#style').value}`;await navigator.clipboard.writeText(text);toast('Suno용 제목·전체 가사·스타일을 모두 복사했습니다')}
...
$('#analyzeStory').onclick=analyze;$('#autoPick').onclick=autoPick;$('#playMix').onclick=playMix;$('#stopMix').onclick=stop;$('#createSong').onclick=makeLyrics;$('#saveSong').onclick=save;$('#downloadTxt').onclick=downloadTxt;$('#copySuno').onclick=copySuno;$('#createScore').onclick=createScore;$('#playScore').onclick=playScore;$('#stopScore').onclick=stop;$('#printScore').onclick=()=>{if(!$('#scoreSvg'))return toast('먼저 악보를 만들어 주세요');window.print()};$('#downloadScore').onclick=downloadScore;$('#scoreMode').onchange=updateScoreMode;['manualKey','manualMeter','manualBpm','dynamics','rhythmMode'].forEach(id=>$('#'+id).onchange=()=>{if($('#scoreSvg'))createScore()});$('#blend').oninput=e=>$('#blendValue').textContent=e.target.value+'%';
...
renderWordChoices();renderRegions();renderInstruments();autoPick();renderSaved();updateScoreMode();if('serviceWorker'in navigator)navigator.serviceWorker.register('sw.js?v=0.6.1');
