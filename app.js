...
// V0.22.43 — dedicated professional-reference microphone capture.
...
function estimateSourcePlaybackBoost(buffer){
 ...
 // V0.22.43: 조용한 녹음/원곡도 실제 청취 음량이 확보되도록 활성 구간 평균을 기준으로 보정한다.
...
function batchReportBlob(rows){const lines=['MARU WORLD MUSIC MAKER V0.22.43 · 원본 보호 자동 처리 보고서','원본 파일은 절대 수정하지 않습니다. 처리본은 별도 WAV로 생성하며, 원본 백업을 선택한 경우 01-ORIGINAL 폴더에 원본 바이트를 그대로 넣습니다.','Suno/완성 마스터 판정 곡은 재압축하지 않고 초미세 EQ와 필요 시 피크 하향만 적용합니다.',''];rows.forEach((r,i)=>lines.push(`${i+1}. ${r.original}\n   길이 ${formatSongDuration(r.duration)} · ${r.rate}Hz · ${r.channels===1?'모노':'스테레오'} · ${r.summary}\n   입력 RMS ${r.beforeRmsDb.toFixed(1)} dBFS · 입력 피크 ${r.beforePeakDb.toFixed(1)} dBFS\n   출력 RMS ${r.afterRmsDb.toFixed(1)} dBFS · 출력 피크 ${r.afterPeakDb.toFixed(1)} dBFS · 전체 레벨 ${r.gainDb>=0?'+':''}${r.gainDb.toFixed(1)} dB\n`));return new Blob(['\ufeff'+lines.join('\n')],{type:'text/plain;charset=utf-8'})}
...
※ V0.22.43의 전문 기준 분석은 ITU-R BS.1770/EBU R128/IEC 음성 명료도 개념을 참고한 브라우저용 비교 분석이며 인증 계측기를 대체하지 않습니다. AI 음질개선은 전문 기준을 강제 EQ 목표로 사용하지 않습니다.`;
...
async function playImportedOriginal(){
 ...
 // V0.22.43: 분석된 원곡은 EQ/압축 없이 피크 안전 범위의 전체 gain만 적용한다.
...
// V0.22.43 — original/analyzed A-B listening + broadcast automatic chain.
...
let audiencePopupCoverUrl='',audiencePopupVideoUrl='',audienceMediaLoadToken=0;
...
function broadcastMediaInfo(id){return broadcastTrackMedia.get(id)||{coverName:'',videoName:''}}
function setBroadcastMediaInfo(id,record={}){if(!id)return;broadcastTrackMedia.set(id,{coverName:record.coverName||'',videoName:record.videoName||'',coverSize:Number(record.coverBlob?.size||0),videoSize:Number(record.videoBlob?.size||0)})}
...
async function applyBroadcastTrackMedia(index){
 revokeTrackAudienceUrls();
 const id=broadcastCurrentIds()[index];
 audienceTrackId='';
 if(!id)return {cover:'',video:''};
 try{
  const r=await broadcastDbGet(id);
  if(r){
   setBroadcastMediaInfo(id,r);
   if(r.videoBlob?.size)audienceTrackVideoUrl=URL.createObjectURL(r.videoBlob);
   if(r.coverBlob?.size)audienceTrackCoverUrl=URL.createObjectURL(r.coverBlob);
   // Only point the audience at the song record when that song actually has media.
   if(r.videoBlob?.size||r.coverBlob?.size)audienceTrackId=id;
  }
  // V0.22.43: if this song has no own media, keep the saved global/default video as the fallback.
  if(!audienceTrackVideoUrl&&!audienceTrackCoverUrl){
   const g=await broadcastDbGet('__audience_global_video__');
   if(g?.videoBlob?.size){
    audienceTrackId='__audience_global_video__';
    audienceTrackVideoUrl=URL.createObjectURL(g.videoBlob);
   }
  }
  return {cover:audienceTrackCoverUrl,video:audienceTrackVideoUrl};
 }catch(e){console.warn('load track media',e);return {cover:'',video:''}}
}
...
// V0.22.43 — separate audience display from operator controls.
...
function revokeAudiencePopupMediaUrls(){
 ...
 audiencePopupCoverUrl='';audiencePopupVideoUrl='';
}
async function audienceLoadTrackMedia(trackId){
 const token=++audienceMediaLoadToken;
 try{
  let r=trackId?await broadcastDbGet(trackId):null;
  // V0.22.43: no per-song video/cover -> load the saved default video record.
  if(!r?.coverBlob?.size&&!r?.videoBlob?.size){
   const g=await broadcastDbGet('__audience_global_video__');
   if(g?.videoBlob?.size)r=g;
  }
  if(token!==audienceMediaLoadToken)return {cover:'',video:''};
  ...
  return {cover:audiencePopupCoverUrl,video:audiencePopupVideoUrl};
 }catch(e){console.warn('audience track media load',e);revokeAudiencePopupMediaUrls();return {cover:'',video:''};}
}
async function audienceRenderMedia(state,vid,img,ph,vidBg,imgBg){
 ...
 // V0.22.43: mobile/BIGO popups load per-song Blob from shared IndexedDB.
 // Show a blurred background fill behind the contain-fitted main media.
 if(state.trackId){const own=await audienceLoadTrackMedia(state.trackId);cover=own.cover||cover;video=own.video||video;}
 const hideBg=()=>{try{vidBg&&vidBg.pause()}catch{}; if(vidBg){vidBg.removeAttribute('src');try{vidBg.load()}catch{};vidBg.style.display='none';} if(imgBg){imgBg.removeAttribute('src');imgBg.style.display='none';}};
 if(video){
  try{
    if(vid.src!==video){vid.pause();vid.src=video;vid.load();}
    vid.muted=true;vid.loop=true;vid.playsInline=true;vid.style.display='block';img.style.display='none';ph.style.display='none';
    if(vidBg){if(vidBg.src!==video){try{vidBg.pause()}catch{};vidBg.src=video;vidBg.load();}vidBg.muted=true;vidBg.loop=true;vidBg.playsInline=true;vidBg.style.display='block';}
    if(imgBg){imgBg.style.display='none';imgBg.removeAttribute('src');}
    await vid.play();
    try{vidBg&&await vidBg.play()}catch{}
  }
  catch(e){
    console.warn('audience video play',e);
    try{vid.muted=true;await vid.play()}catch{};
    if(cover){
      img.src=cover;img.style.display='block';ph.style.display='none';
      if(imgBg){imgBg.src=cover;imgBg.style.display='block';}
      if(vidBg){try{vidBg.pause()}catch{};vidBg.style.display='none';vidBg.removeAttribute('src');}
    } else hideBg();
  }
 }else{
  ...
  if(cover){
    img.src=cover;img.style.display='block';ph.style.display='none';
    if(imgBg){imgBg.src=cover;imgBg.style.display='block';}
    if(vidBg){try{vidBg.pause()}catch{};vidBg.removeAttribute('src');try{vidBg.load()}catch{};vidBg.style.display='none';}
  }else{
    img.removeAttribute('src');img.style.display='none';ph.style.display='flex';hideBg();
  }
 }
}
async function applyAudienceState(state={}){const title=$('#audienceTitle'),img=$('#audienceCover'),vid=$('#audienceVideo'),ph=$('#audienceCoverPlaceholder'),vidBg=$('#audienceVideoBg'),imgBg=$('#audienceCoverBg');if(title)title.textContent=state.title||'방송 대기';await audienceRenderMedia(state,vid,img,ph,vidBg,imgBg)}
function publishAudienceState(partial={}){const prev=readAudienceState();const state={...prev,title:prev.title||'방송 대기',status:prev.status||'MARU MUSIC LIVE',message:audienceText()||prev.message||'',trackId:audienceTrackId||prev.trackId||'',cover:audienceTrackCoverUrl||audienceCoverData||prev.cover||'',video:audienceTrackVideoUrl||audienceVideoUrl||prev.video||'',...partial,updatedAt:Date.now()};try{localStorage.setItem(AUDIENCE_STATE_KEY,JSON.stringify({...state,cover:audienceCoverData||'',video:''}))}catch{}try{audienceChannel?.postMessage(state)}catch{}applyAudienceState(state);return state}
window.addEventListener('beforeunload',()=>{revokeAudiencePopupMediaUrls()});
function setupAudienceMode(){const audience=new URLSearchParams(location.search).get('mode')==='audience';document.body.classList.toggle('audience-mode',audience);applyAudienceState(readAudienceState());try{audienceChannel&&(audienceChannel.onmessage=e=>applyAudienceState(e.data||{}))}catch{}window.addEventListener('storage',e=>{if(e.key===AUDIENCE_STATE_KEY)applyAudienceState(readAudienceState())})}
...
async function loadAudienceVideo(file){if(!file)return;try{let r=await broadcastDbGet('__audience_global_video__')||{id:'__audience_global_video__',name:'MARU viewer video',blob:new Blob([]),type:'application/x-maru-media'};r.videoBlob=file;r.videoName=file.name||'video';r.mediaUpdatedAt=Date.now();await broadcastDbPut(r)}catch(e){console.warn('global audience video store',e)}if(audienceVideoUrl){try{URL.revokeObjectURL(audienceVideoUrl)}catch{}}audienceVideoUrl=URL.createObjectURL(file);audienceTrackId='__audience_global_video__';try{localStorage.setItem('maru-default-video-enabled-v1','1')}catch{};publishAudienceState({trackId:audienceTrackId,video:audienceVideoUrl,cover:''});toast('기본 동영상을 저장했습니다')}
...
function renderBroadcastQueue(){const box=$('#broadcastQueue');if(!box){updateBroadcastSelectionActions();updateBroadcastDock();return}const rows=broadcastVisibleRows(),favs=readBroadcastFavs(),ids=broadcastCurrentIds();box.innerHTML=rows.length?rows.map(x=>{const selected=broadcastSelectedIds.has(x.id),canUp=x.i>0,canDown=x.i<broadcastFiles.length-1,media=broadcastMediaInfo(x.id),hasCover=!!media.coverName,hasVideo=!!media.videoName;return `<div class="qrow ${broadcastRunning&&x.i===broadcastIndex?'playing':''} ${selected?'selected':''}"><button class="qplay" type="button" data-broadcast-play="${x.i}" aria-label="${esc(x.title)} 재생">▶</button><input class="qselect" type="checkbox" data-broadcast-select="${x.i}" ${selected?'checked':''} aria-label="${esc(x.title)} 선택"><span class="qnum">${x.i+1}</span><span class="qtitle" title="${esc(x.title)}">${esc(x.title)}</span><span class="qmedia"><button class="${hasCover?'active':''}" type="button" data-broadcast-cover="${x.i}" title="${hasCover?esc(media.coverName):'커버 등록'}"><span>🖼</span><b>커버</b></button><button class="${hasVideo?'active':''}" type="button" data-broadcast-video="${x.i}" title="${hasVideo?esc(media.videoName):'동영상 등록'}"><span>🎬</span><b>영상</b></button><button type="button" data-broadcast-media-preview="${x.i}" aria-label="미리보기"><span>👁</span><b>보기</b></button>${hasCover||hasVideo?`<button class="media-clear" type="button" data-broadcast-media-clear="${x.i}" aria-label="커버 영상 연결 삭제"><span>×</span><b>해제</b></button>`:''}</span><span class="qmove"><button type="button" data-broadcast-up="${x.i}" ${canUp?'':'disabled'} aria-label="위로 이동">▲</button><button type="button" data-broadcast-down="${x.i}" ${canDown?'':'disabled'} aria-label="아래로 이동">▼</button></span><button class="qfav ${favs.has(x.f.name)?'active':''}" type="button" data-broadcast-fav="${x.i}" aria-label="즐겨찾기">${favs.has(x.f.name)?'★':'☆'}</button><button class="qdel" type="button" data-broadcast-delete="${x.i}" aria-label="${esc(x.title)} 삭제">🗑</button></div>`}).join(''):`<div class="analysis-box">조건에 맞는 노래가 없습니다.</div>`;box.querySelectorAll('[data-broadcast-play]').forEach(b=>b.onclick=()=>broadcastJumpTo(Number(b.dataset.broadcastPlay)));box.querySelectorAll('[data-broadcast-select]').forEach(b=>b.onchange=()=>{const i=Number(b.dataset.broadcastSelect),id=ids[i];if(!id)return;if(b.checked)broadcastSelectedIds.add(id);else broadcastSelectedIds.delete(id);b.closest('.qrow')?.classList.toggle('selected',b.checked);updateBroadcastSelectionActions()});box.querySelectorAll('[data-broadcast-cover]').forEach(b=>b.onclick=()=>chooseBroadcastTrackMedia(Number(b.dataset.broadcastCover),'cover'));box.querySelectorAll('[data-broadcast-video]').forEach(b=>b.onclick=()=>chooseBroadcastTrackMedia(Number(b.dataset.broadcastVideo),'video'));box.querySelectorAll('[data-broadcast-media-preview]').forEach(b=>b.onclick=()=>previewBroadcastTrackMedia(Number(b.dataset.broadcastMediaPreview)));box.querySelectorAll('[data-broadcast-media-clear]').forEach(b=>b.onclick=()=>clearBroadcastTrackMedia(Number(b.dataset.broadcastMediaClear)));box.querySelectorAll('[data-broadcast-up]').forEach(b=>b.onclick=()=>moveBroadcastTrack(Number(b.dataset.broadcastUp),-1));box.querySelectorAll('[data-broadcast-down]').forEach(b=>b.onclick=()=>moveBroadcastTrack(Number(b.dataset.broadcastDown),1));box.querySelectorAll('[data-broadcast-delete]').forEach(b=>b.onclick=()=>deleteOneBroadcastTrack(Number(b.dataset.broadcastDelete)));box.querySelectorAll('[data-broadcast-fav]').forEach(b=>b.onclick=()=>{const i=Number(b.dataset.broadcastFav),f=broadcastFiles[i],set=readBroadcastFavs();if(!f)return;if(set.has(f.name))set.delete(f.name);else set.add(f.name);saveBroadcastFavs(set);renderBroadcastQueue()});updateBroadcastSelectionActions();updateBroadcastDock()}
async function selectBroadcastFiles(files){broadcastStop(false);const list=[...(files||[])].filter(f=>f.type?.startsWith('audio/')||/\.(mp3|wav|m4a|aac|ogg|flac)$/i.test(f.name)).slice(0,100);broadcastFiles=list;refreshBroadcastListUi(list.length?`${list.length}곡 준비 · 브라우저에 자동 저장 중…`:'방송할 곡을 선택하세요.');renderLearningProfile();if(!list.length)return;try{await persistBroadcastPlaylist(list,{append:false});broadcastTrackIds=broadcastOrderRead();await updateBroadcastPersistStatus(`${list.length}곡 자동 저장 완료 · 다음 방송 때 목록 자동 복원`);renderLearningProfile();queueBroadcastAutoLearning(list,{delay:400});toast(`방송목록 ${list.length}곡을 자동 저장했습니다`)}catch(e){console.error('persist broadcast playlist',e);await updateBroadcastPersistStatus('현재 방송에서는 사용 가능 · 영구 저장은 실패했습니다');queueBroadcastAutoLearning(list,{delay:400});toast('방송목록 자동 저장에 실패했습니다. 브라우저 저장공간을 확인해 주세요.')}}
function speakBroadcast(text){text=String(text||'').trim();if(!text||!('speechSynthesis'in window))return Promise.resolve();return new Promise(resolve=>{try{speechSynthesis.cancel();const u=new SpeechSynthesisUtterance(text);u.lang='ko-KR';u.rate=.96;u.pitch=1;u.volume=1;let done=false;const finish=()=>{if(done)return;done=true;resolve()};u.onend=finish;u.onerror=finish;speechSynthesis.speak(u);setTimeout(finish,Math.max(3500,text.length*170))}catch{resolve()}})}
async function broadcastTransitionToNext(manual=false){if(!broadcastRunning||broadcastTransitioning||broadcastPaused)return;broadcastTransitioning=true;try{let next=broadcastIndex+1;if(next>=broadcastFiles.length){if($('#broadcastLoop')?.checked)next=0;else{return broadcastStop(true)}}const nextTitle=broadcastSafeTitle(broadcastFiles[next].name);if(!manual){const custom=$('#broadcastCustomMessage')?.value?.trim();if($('#broadcastCustomEnabled')?.checked&&custom){$('#broadcastNow').textContent=`멘트 재생 중 · ${custom}`;publishAudienceState({status:'안내 중',message:custom});await speakBroadcast(custom)}const tpl=$('#broadcastNextTemplate')?.value||'다음 곡은 {title}입니다.';if($('#broadcastNextEnabled')?.checked){const text=tpl.replaceAll('{title}',nextTitle);$('#broadcastNow').textContent=`다음곡 안내 · ${text}`;publishAudienceState({title:nextTitle,status:'다음 곡 안내',message:text});await speakBroadcast(text)}}if(!broadcastRunning||broadcastPaused)return;await playBroadcastIndex(next)}finally{broadcastTransitioning=false;updateBroadcastDock()}}
async function playBroadcastIndex(i){if(!broadcastRunning||broadcastPaused||!broadcastFiles[i])return;const a=$('#broadcastAudio');if(!a)return;broadcastIndex=i;renderBroadcastQueue();if(broadcastUrls[i])URL.revokeObjectURL(broadcastUrls[i]);broadcastUrls[i]=URL.createObjectURL(broadcastFiles[i]);a.src=broadcastUrls[i];a.load();const nowTitle=broadcastSafeTitle(broadcastFiles[i].name);markBroadcastRecent(broadcastFiles[i].name);$('#broadcastNow').textContent=`▶ ${i+1}/${broadcastFiles.length} · ${nowTitle}`;$('#broadcastBadge').textContent=`재생 ${i+1}/${broadcastFiles.length}`;await applyBroadcastTrackMedia(i);publishAudienceState({title:nowTitle,status:`재생 중 · ${i+1}/${broadcastFiles.length}`,message:audienceText()});try{await a.play()}catch(e){console.warn('broadcast play',e);toast('브라우저가 자동 재생을 막았습니다. 방송 시작을 다시 눌러 주세요.')}}
async function broadcastJumpTo(i){if(!broadcastFiles[i])return;if(!broadcastRunning){broadcastRunning=true;broadcastPaused=false;$('#broadcastStop').disabled=false;$('#broadcastPauseBtn')&&($('#broadcastPauseBtn').disabled=false)}else{broadcastPaused=false;try{speechSynthesis?.cancel?.()}catch{}const a=$('#broadcastAudio');if(a)a.pause()}await playBroadcastIndex(i);updateBroadcastDock()}
async function broadcastPrevious(){if(!broadcastFiles.length)return pickBroadcastSongs();const was=broadcastIndex;let prev=was>0?was-1:0;if(was<0)prev=0;if(!broadcastRunning)return broadcastJumpTo(prev);broadcastPaused=false;try{speechSynthesis?.cancel?.()}catch{}const a=$('#broadcastAudio');if(a)a.pause();await playBroadcastIndex(prev);toast(was<=0?'첫 곡을 처음부터 다시 재생합니다':'이전 곡으로 돌아갑니다')}
async function broadcastStart(){if(!broadcastFiles.length)return toast('방송할 노래를 먼저 선택해 주세요');broadcastStop(false);broadcastRunning=true;broadcastPaused=false;$('#broadcastStart').disabled=true;$('#broadcastSkip').disabled=false;$('#broadcastStop').disabled=false;$('#broadcastBadge').textContent='시작';await playBroadcastIndex(0);updateBroadcastDock()}
async function broadcastStartOrResume(){if(!broadcastFiles.length)return toast('방송할 노래를 먼저 선택해 주세요');if(!broadcastRunning)return broadcastStart();if(!broadcastPaused)return;broadcastPaused=false;const a=$('#broadcastAudio');try{speechSynthesis?.resume?.()}catch{}if(a&&a.src&&broadcastIndex>=0&&Number.isFinite(a.duration)&&a.currentTime<a.duration-.05){try{await a.play()}catch{}}else{await broadcastTransitionToNext(true)}if($('#broadcastBadge'))$('#broadcastBadge').textContent=`재생 ${Math.max(1,broadcastIndex+1)}/${broadcastFiles.length}`;updateBroadcastDock()}
function broadcastPause(){if(!broadcastRunning||broadcastPaused)return;broadcastPaused=true;try{speechSynthesis?.cancel?.()}catch{}const a=$('#broadcastAudio');if(a)a.pause();if($('#broadcastNow'))$('#broadcastNow').textContent=`⏸ 방송 대기 · ${broadcastIndex>=0?broadcastSafeTitle(broadcastFiles[broadcastIndex]?.name):''}`;if($('#broadcastBadge'))$('#broadcastBadge').textContent='방송 대기';publishAudienceState({status:'방송 대기'});updateBroadcastDock()}
function broadcastStop(showToast=true){broadcastRunning=false;broadcastTransitioning=false;broadcastPaused=false;revokeTrackAudienceUrls();try{speechSynthesis?.cancel?.()}catch{}const a=$('#broadcastAudio');if(a){a.pause();a.removeAttribute('src');a.load()}broadcastUrls.forEach(u=>{try{u&&URL.revokeObjectURL(u)}catch{}});broadcastUrls=[];broadcastIndex=-1;const start=$('#broadcastStart'),skip=$('#broadcastSkip'),stopb=$('#broadcastStop'),prev=$('#broadcastPrev'),pause=$('#broadcastPauseBtn');if(start)start.disabled=!broadcastFiles.length;if(skip)skip.disabled=!broadcastFiles.length;if(prev)prev.disabled=!broadcastFiles.length;if(pause)pause.disabled=true;if(stopb)stopb.disabled=true;if($('#broadcastBadge'))$('#broadcastBadge').textContent=broadcastFiles.length?`${broadcastFiles.length}곡`:'대기';renderBroadcastQueue();publishAudienceState({title:'방송 종료',status:'MARU MUSIC LIVE',message:''});if(showToast)toast('방송 이어재생을 정지했습니다')}
...
$('#abOriginal').onclick=playABOriginal;$('#abAnalyzed').onclick=playABAnalyzed;$('#abStop').onclick=stopAB;$('#broadcastFiles').onchange=e=>selectBroadcastFiles(e.target.files);$('#selectVisibleBroadcast').onclick=selectVisibleBroadcastTracks;$('#clearBroadcastSelection').onclick=clearBroadcastTrackSelection;$('#deleteSelectedBroadcast').onclick=deleteSelectedBroadcastTracks;$('#clearBroadcastSaved').onclick=clearSavedBroadcastPlaylist;$('#broadcastStart').onclick=broadcastStartOrResume;$('#broadcastPauseBtn').onclick=broadcastPause;$('#broadcastPrev').onclick=broadcastPrevious;$('#broadcastSkip').onclick=()=>broadcastTransitionToNext(true);$('#broadcastStop').onclick=()=>broadcastStop(true);$('#broadcastSearch').oninput=renderBroadcastQueue;$('#broadcastFavOnly').onchange=renderBroadcastQueue;$('#broadcastListSort').onchange=renderBroadcastQueue;$('#broadcastAudio').onended=()=>broadcastTransitionToNext(false);$('#broadcastAudio').onerror=()=>{if(broadcastRunning){toast('이 곡을 재생하지 못해 다음 곡으로 넘어갑니다');broadcastTransitionToNext(true)}};$('#broadcastToggle').onclick=toggleBroadcastCard;$('#openAudienceView').onclick=openAudienceView;$('#broadcastCoverFile').onchange=e=>loadAudienceCover(e.target.files?.[0]);$('#broadcastVideoFile').onchange=e=>loadAudienceVideo(e.target.files?.[0]);$('#broadcastAudienceText').oninput=()=>{publishAudienceState({message:audienceText()});saveBroadcastSettings()};['broadcastCustomMessage','broadcastNextTemplate'].forEach(id=>{const el=$('#'+id);if(el)el.oninput=saveBroadcastSettings});['broadcastCustomEnabled','broadcastNextEnabled','broadcastLoop'].forEach(id=>{const el=$('#'+id);if(el)el.onchange=saveBroadcastSettings});
...
$('#dockBroadcastStart').onclick=dockStart;$('#dockBroadcastPause').onclick=dockPause;$('#dockBroadcastPrev').onclick=dockPrev;$('#dockBroadcastNext').onclick=dockNext;$('#dockBroadcastEnd').onclick=dockEnd;restoreBroadcastSettings();setupAudienceMode();updateBroadcastDock();restoreBroadcastPlaylist().catch(e=>{console.warn('broadcast restore',e);updateBroadcastPersistStatus('자동 복원 실패 · 노래를 한 번 다시 선택해 주세요.')});
...
/* V0.22.43 ensure popup control styles */
(function(){
...
/* V0.22.43 friendly end greeting */
(function(){
...
/* V0.22.43 direct audience touch controls */
(function(){
...
/* V0.22.43 installable fullscreen PWA + forced browser gate */
(function(){
...
/* V0.22.43 self-contained audience PWA bootstrap */
(function(){
