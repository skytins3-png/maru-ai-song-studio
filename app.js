...
let region='한국',selected=new Set(),chosenWords=new Set(),ctx=null,timers=[],lastProfile=PROFILES.generic,scoreNotes=[];
...
const KEY_CHORDS={C:['C','G','Am','F'],D:['D','A','Bm','G'],E:['E','B','C#m','A'],F:['F','C','Dm','Bb'],'F#':['F#','C#','D#m','B'],G:['G','D','Em','C'],A:['A','E','F#m','D'],B:['B','F#','G#m','E']};
function esc(s){return String(s).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]))}
function createScore(){
 if(!$('#lyrics').value){toast('먼저 노래 설계를 만들어 주세요');return}const p=activeProfile(),key=p.key.split(' ')[0],minor=p.key.includes('Minor'),root=KEY_ROOTS[key]||67,scale=minor?[0,2,3,5,7,8,10]:[0,2,4,5,7,9,11],six=p.meter==='6/8',perBar=six?6:4,patterns=six?[0,2,4,3,2,1,1,3,5,4,3,2,2,4,6,5,4,2]:[0,2,4,3,1,3,5,4,2,4,6,5,3,2,1,0],entries=[];let section='';
 $('#lyrics').value.split('\n').forEach(raw=>{const line=raw.trim();if(!line)return;if(/^\[.+\]$/.test(line)){section=line.slice(1,-1);return}entries.push({section,text:line.replace(/^\(|\)$/g,''),instrumental:/^\(.+\)$/.test(line)})});const bars=entries.length,systems=Math.ceil(bars/4),height=65+systems*145+30;scoreNotes=[];
 for(let bar=0;bar<bars;bar++)for(let beat=0;beat<perBar;beat++){const raw=patterns[(bar*perBar+beat+bar)%patterns.length],oct=raw>=7?1:0,degree=raw%7,midi=root+scale[degree]+oct*12;scoreNotes.push({midi,degree:degree+oct*7,bar,beat})}
 const chords=KEY_CHORDS[key]||KEY_CHORDS.G;let body='';for(let system=0;system<systems;system++){const top=65+system*145,left=70,right=900;for(let l=0;l<5;l++)body+=`<line x1="${left}" y1="${top+l*12}" x2="${right}" y2="${top+l*12}" stroke="#222"/>`;body+=`<text x="25" y="${top+42}" font-size="52">𝄞</text><text x="58" y="${top+22}" font-size="18">${six?'6':'4'}</text><text x="58" y="${top+42}" font-size="18">${six?'8':'4'}</text>`;for(let b=0;b<4;b++){const bar=system*4+b;if(bar>=bars)break;const item=entries[bar],x0=75+b*207.5,previous=entries[bar-1];body+=`<line x1="${x0}" y1="${top}" x2="${x0}" y2="${top+48}" stroke="#222"/>`;if(!previous||previous.section!==item.section)body+=`<text x="${x0+5}" y="${top-25}" font-size="12" font-weight="800" fill="#7040a0">${esc(item.section)}</text>`;body+=`<text x="${x0+8}" y="${top-8}" font-size="16" font-weight="700">${esc(chords[bar%4])}</text>`;const notes=scoreNotes.filter(n=>n.bar===bar);notes.forEach((n,j)=>{const x=x0+22+j*(175/notes.length),y=top+42-n.degree*6,index=bar*perBar+j;body+=`<g class="score-note" data-note="${index}"><ellipse cx="${x}" cy="${y}" rx="7" ry="5" transform="rotate(-18 ${x} ${y})" fill="#111"/><line x1="${x+6}" y1="${y}" x2="${x+6}" y2="${y-30}" stroke="#111" stroke-width="2"/></g>`});body+=`<text x="${x0+7}" y="${top+75}" font-size="12.5" ${item.instrumental?'font-style="italic" fill="#555"':''}>${esc(item.text)}</text>`}body+=`<line x1="900" y1="${top}" x2="900" y2="${top+48}" stroke="#222" stroke-width="2"/>`}
 const title=esc($('#title').value||'MARU SONG');$('#scoreSheet').innerHTML=`<svg id="scoreSvg" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 930 ${height}" role="img" aria-label="${title} 전체 멜로디 악보"><rect width="930" height="${height}" fill="white"/><text x="465" y="28" text-anchor="middle" font-size="22" font-weight="800">${title} · Full Song Melody</text>${body}<text x="465" y="${height-8}" text-anchor="middle" font-size="12">MARU WORLD MUSIC MAKER V0.5 · 전체 곡 자동 생성 초안</text></svg>`;$('#scoreMeta').textContent=`${p.key} · ${p.meter}박자 · ${p.bpm} BPM · 전체 ${bars}마디 · ${systems}단 악보 · 코드 ${chords.join('–')}`;toast('전체 가사와 전 구간 멜로디 악보를 만들었습니다')
}
function playScore(){if(!scoreNotes.length){createScore();if(!scoreNotes.length)return}stop();const p=activeProfile(),unit=(60/p.bpm)*(p.meter==='6/8'?.5:1);audio().resume();scoreNotes.forEach((n,i)=>{tone('piano',440*Math.pow(2,(n.midi-69)/12),i*unit,unit*.85,.075);timers.push(setTimeout(()=>{$$('.score-note').forEach(x=>x.classList.remove('active'));$(`[data-note="${i}"]`)?.classList.add('active')},i*unit*1000))});timers.push(setTimeout(()=>{$$('.score-note').forEach(x=>x.classList.remove('active'))},scoreNotes.length*unit*1000));toast('악보의 멜로디를 재생합니다')}
...
renderWordChoices();renderRegions();renderInstruments();autoPick();renderSaved();if('serviceWorker'in navigator)navigator.serviceWorker.register('sw.js?v=0.5.0');
