...
// V0.23.11 — byte-exact duplicate cleanup for the saved broadcast playlist.
async function broadcastBlobsEqual2311(a,b){
 const sizeA=Number(a?.size||0),sizeB=Number(b?.size||0);
 if(!sizeA||sizeA!==sizeB||typeof a.slice!=='function'||typeof b.slice!=='function')return false;
 const chunkSize=1024*1024;
 for(let offset=0;offset<sizeA;offset+=chunkSize){
  const end=Math.min(sizeA,offset+chunkSize),[ab,bb]=await Promise.all([a.slice(offset,end).arrayBuffer(),b.slice(offset,end).arrayBuffer()]),av=new Uint8Array(ab),bv=new Uint8Array(bb);
  if(av.length!==bv.length)return false;
  for(let i=0;i<av.length;i++)if(av[i]!==bv[i])return false;
 }
 return true;
}
function mergeDuplicateBroadcastRecord2311(keep,extra){
 let changed=false;
 for(const [blobKey,nameKey] of [['coverBlob','coverName'],['videoBlob','videoName']]){
  if(!keep[blobKey]&&extra?.[blobKey]){keep[blobKey]=extra[blobKey];keep[nameKey]=extra[nameKey]||'';keep.mediaUpdatedAt=Math.max(Number(keep.mediaUpdatedAt||0),Number(extra.mediaUpdatedAt||0));changed=true}
 }
 if(!String(keep.subtitleText||'').trim()&&String(extra?.subtitleText||'').trim()){keep.subtitleText=extra.subtitleText;keep.subtitleUpdatedAt=extra.subtitleUpdatedAt||Date.now();changed=true}
 const first=Math.min(Number(keep.addedAt||Infinity),Number(extra?.addedAt||Infinity));
 if(Number.isFinite(first)&&first!==Number(keep.addedAt||0)){keep.addedAt=first;changed=true}
 return changed;
}
async function dedupeBroadcastPlaylist2311(ids=broadcastOrderRead()){
 const source=[...(ids||[])],keptIds=[],keptRecords=[],seenIds=new Set();let removed=0;
 for(const id of source){
  if(!id)continue;
  if(seenIds.has(id)){removed++;continue}
  seenIds.add(id);
  let record=null;
  try{record=await broadcastDbGet(id)}catch(e){console.warn('duplicate scan read',id,e);keptIds.push(id);keptRecords.push(null);continue}
  if(!record?.blob){keptIds.push(id);keptRecords.push(record||null);continue}
  let match=-1;
  for(let i=0;i<keptRecords.length;i++){
   const candidate=keptRecords[i];
   if(!candidate?.blob||Number(candidate.blob.size)!==Number(record.blob.size))continue;
   let same=false;try{same=await broadcastBlobsEqual2311(candidate.blob,record.blob)}catch(e){console.warn('duplicate byte compare',id,e)}
   if(same){match=i;break}
  }
  if(match<0){keptIds.push(id);keptRecords.push(record);continue}
  const keeper=keptRecords[match];
  try{
   if(mergeDuplicateBroadcastRecord2311(keeper,record))await broadcastDbPut(keeper);
   await broadcastDbDelete(id);removed++;
  }catch(e){console.warn('duplicate auto remove',id,e);keptIds.push(id);keptRecords.push(record)}
 }
...
async function selectBroadcastFiles(files){const incoming=[...(files||[])].filter(broadcastIsPlayableFile).slice(0,100);if(!incoming.length)return toast('MP3·WAV·M4A 또는 MP4·MOV 파일을 선택해 주세요');if(broadcastRunning)broadcastStop(false);const known=new Set(broadcastCurrentIds()),fresh=[];let skipped=0;for(const f of incoming){const id=broadcastFileId(f,f.name);if(!known.has(id)){known.add(id);fresh.push(f)}else skipped++}if(!fresh.length)return toast(`동일한 노래 ${skipped}개는 추가하지 않고 기존 파일 한 개만 유지했습니다`);try{const saved=await persistBroadcastPlaylist(fresh,{append:true}),restored=await restoreBroadcastPlaylist({removedBefore:saved?.removed||0}),removed=skipped+Number(restored?.removed||0),liveIds=new Set(broadcastCurrentIds()),addedFiles=fresh.filter(f=>liveIds.has(broadcastFileId(f,f.name)));await updateBroadcastPersistStatus(`${broadcastFiles.length}개 자동 저장 완료${removed?` · 중복 ${removed}개 자동 제거`:''} · MP3/MP4 통합 목록`);queueBroadcastAutoLearning(addedFiles,{delay:400});toast(removed?`${addedFiles.length}개 추가 · 중복 ${removed}개 자동 제거 · 같은 노래는 한 개만 유지`:`${addedFiles.length}개를 같은 방송목록에 추가했습니다`)}catch(e){console.error('persist broadcast playlist',e);toast('방송목록 저장에 실패했습니다. 브라우저 저장공간을 확인해 주세요.')}}
