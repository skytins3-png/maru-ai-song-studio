...
// V0.22.61 — dedicated professional-reference microphone capture.
...
function estimateSourcePlaybackBoost(buffer){
 ...
 // V0.22.61: 조용한 녹음/원곡도 실제 청취 음량이 확보되도록 활성 구간 평균을 기준으로 보정한다.
...
function batchReportBlob(rows){const lines=['MARU WORLD MUSIC MAKER V0.22.61 · 원본 보호 자동 처리 보고서','원본 파일은 절대 수정하지 않습니다. MP4는 음성 트랙을 분석해 별도 WAV 처리본으로 생성하며, 원본 백업을 선택한 경우 01-ORIGINAL 폴더에 원본 MP4 바이트를 그대로 넣습니다.','Suno/완성 마스터 판정 곡은 재압축하지 않고 초미세 EQ와 필요 시 피크 하향만 적용합니다.',''];rows.forEach((r,i)=>lines.push(`${i+1}. ${r.original}\n   길이 ${formatSongDuration(r.duration)} · ${r.rate}Hz · ${r.channels===1?'모노':'스테레오'} · ${r.summary}\n   입력 RMS ${r.beforeRmsDb.toFixed(1)} dBFS · 입력 피크 ${r.beforePeakDb.toFixed(1)} dBFS\n   출력 RMS ${r.afterRmsDb.toFixed(1)} dBFS · 출력 피크 ${r.afterPeakDb.toFixed(1)} dBFS · 전체 레벨 ${r.gainDb>=0?'+':''}${r.gainDb.toFixed(1)} dB\n`));return new Blob(['\ufeff'+lines.join('\n')],{type:'text/plain;charset=utf-8'})}
...
※ V0.22.61의 전문 기준 분석은 ITU-R BS.1770/EBU R128/IEC 음성 명료도 개념을 참고한 브라우저용 비교 분석이며 인증 계측기를 대체하지 않습니다. AI 음질개선은 전문 기준을 강제 EQ 목표로 사용하지 않습니다.`;
...
async function playImportedOriginal(){
 ...
 // V0.22.61: 분석된 원곡은 EQ/압축 없이 피크 안전 범위의 전체 gain만 적용한다.
...
// V0.22.61 — original/analyzed A-B listening + broadcast automatic chain.
...
// V0.22.61 — separate audience display from operator controls.
...
/* V0.22.61 FINAL — karaoke progress and Chinese/Korean pronunciation */
...
async function audienceLoadTrackMedia(trackId,force=false){
 ...
 // V0.22.61: keep the same object URLs while subtitle/title/status changes.
...
async function audienceRenderMedia(state,vid,img,ph,vidBg,imgBg){
 ...
 // V0.22.61: only ONE video decoder. The former blurred background video is never played.
...
let audienceRenderedTrackKey2261='',audienceRenderedDirectVideo2261='',audienceRenderedDirectCover2261='';
async function applyAudienceState(state={}){
 ...
 const trackKey=String(state.trackId||'');
 const directVideo=String(state.video||'');
 const directCover=String(state.cover||'');
 const sameMedia=trackKey===audienceRenderedTrackKey2261&&directVideo===audienceRenderedDirectVideo2261&&directCover===audienceRenderedDirectCover2261;
 // Subtitle/title/status updates are frequent. Do not touch the video element when media did not change.
 if(sameMedia&&vid&&(vid.currentSrc||vid.src||img?.src))return;
 audienceRenderedTrackKey2261=trackKey;
 audienceRenderedDirectVideo2261=directVideo;
 audienceRenderedDirectCover2261=directCover;
...
/* V0.22.61 automatic subtitle fallback (browser speech recognition) */
...
function publishAutoCaptionText(text){broadcastAutoCaptionText=String(text||'').trim();clearTimeout(autoCaptionFallbackTimer2260);const enabledEl=$('#broadcastSubtitleEnabled');if(enabledEl&&broadcastAutoCaptionText)enabledEl.checked=true;const base=audienceSubtitleState();publishAudienceState({subtitle:{...base,enabled:!!broadcastAutoCaptionText,text:broadcastAutoCaptionText,auto:true,captionAt:Date.now(),titleFallback:false}})}
...
async function broadcastStart(startIndex=null){
 ...
 // V0.22.61: do not call broadcastStop(false) here. It published the default-video/ended state
...
/* V0.22.61 ensure popup control styles */
(function(){
...
/* V0.22.61 friendly end greeting */
(function(){
...
/* V0.22.61 direct audience touch controls */
(function(){
...
/* V0.22.61 installable fullscreen PWA + forced browser gate */
(function(){
...
/* V0.22.61 self-contained audience PWA bootstrap */
(function(){
...
/* V0.22.61 live canvas sidebar fill — one video decoder only */
(function(){
...
/* V0.22.61 keep auto captions aligned with playback state */
(function(){
...
/* V0.22.61 subtitle auto-enable migration + safe title fallback */
...
function scheduleAutoCaptionFallback2260(title){
  ...
  autoCaptionFallbackTimer2260=setTimeout(()=>{
    try{
      ...
      publishAudienceState({subtitle:{...base,enabled:true,text:String(title||'').trim(),auto:true,captionAt:Date.now(),titleFallback:true}});
      setBroadcastAutoCaptionStatus('🎵 자동 가사 인식 대기 · 현재는 곡 제목을 표시 중','warn');
    }catch(e){}
  },2200);
}

/* V0.22.61 stable-media invariant: subtitle/title/status updates must never reload unchanged audience video. */
