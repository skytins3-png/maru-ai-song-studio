...
// V0.22.68 — dedicated professional-reference microphone capture.
...
function estimateSourcePlaybackBoost(buffer){
 ...
 // V0.22.68: 조용한 녹음/원곡도 실제 청취 음량이 확보되도록 활성 구간 평균을 기준으로 보정한다.
...
function batchReportBlob(rows){const lines=['MARU WORLD MUSIC MAKER V0.22.68 · 원본 보호 자동 처리 보고서','원본 파일은 절대 수정하지 않습니다. MP4는 음성 트랙을 분석해 별도 WAV 처리본으로 생성하며, 원본 백업을 선택한 경우 01-ORIGINAL 폴더에 원본 MP4 바이트를 그대로 넣습니다.','Suno/완성 마스터 판정 곡은 재압축하지 않고 초미세 EQ와 필요 시 피크 하향만 적용합니다.',''];rows.forEach((r,i)=>lines.push(`${i+1}. ${r.original}\n   길이 ${formatSongDuration(r.duration)} · ${r.rate}Hz · ${r.channels===1?'모노':'스테레오'} · ${r.summary}\n   입력 RMS ${r.beforeRmsDb.toFixed(1)} dBFS · 입력 피크 ${r.beforePeakDb.toFixed(1)} dBFS\n   출력 RMS ${r.afterRmsDb.toFixed(1)} dBFS · 출력 피크 ${r.afterPeakDb.toFixed(1)} dBFS · 전체 레벨 ${r.gainDb>=0?'+':''}${r.gainDb.toFixed(1)} dB\n`));return new Blob(['\ufeff'+lines.join('\n')],{type:'text/plain;charset=utf-8'})}
...
※ V0.22.68의 전문 기준 분석은 ITU-R BS.1770/EBU R128/IEC 음성 명료도 개념을 참고한 브라우저용 비교 분석이며 인증 계측기를 대체하지 않습니다. AI 음질개선은 전문 기준을 강제 EQ 목표로 사용하지 않습니다.`;
...
async function playImportedOriginal(){
 ...
 // V0.22.68: 분석된 원곡은 EQ/압축 없이 피크 안전 범위의 전체 gain만 적용한다.
...
// V0.22.68 — original/analyzed A-B listening + broadcast automatic chain.
...
// V0.22.68 — separate audience display from operator controls.
...
/* V0.22.68 FINAL — karaoke progress and Chinese/Korean pronunciation */
...
async function audienceLoadTrackMedia(trackId,force=false){
 ...
 // V0.22.68: keep the same object URLs while subtitle/title/status changes.
...
async function audienceRenderMedia(state,vid,img,ph,vidBg,imgBg){
 ...
 // V0.22.68: only ONE video decoder. The former blurred background video is never played.
 ...
 if(video){
  try{
   ...
   // V0.22.68: no canvas mirror/backdrop loop during broadcast. The video itself fills the popup.
...
/* V0.22.68 automatic subtitle fallback (browser speech recognition) */
...
async function broadcastStart(startIndex=null){
 ...
 // V0.22.68: do not call broadcastStop(false) here. It published the default-video/ended state
...
/* V0.22.68 ensure popup control styles */
(function(){
...
/* V0.22.68 friendly end greeting */
(function(){
...
/* V0.22.68 direct audience touch controls */
(function(){
...
/* V0.22.68 installable fullscreen PWA + forced browser gate */
(function(){
...
/* V0.22.68 self-contained audience PWA bootstrap */
(function(){
...
/* V0.22.68 live canvas sidebar fill — one video decoder only */
(function(){
...
/* V0.22.68 keep auto captions aligned with playback state */
(function(){
...
/* V0.22.68 subtitle auto-enable migration + safe title fallback */
...
function scheduleAutoCaptionFallback2260(title){
  ...
  autoCaptionFallbackTimer2260=setTimeout(()=>{
    try{
      ...
      // V0.22.68: never promote the song title into the karaoke subtitle layer.
...
/* V0.22.68 stable-media invariant: subtitle/title/status updates must never reload unchanged audience video. */


/* V0.22.68 clear legacy title-as-subtitle state */
(function(){
  try{
...
/* V0.22.68 stable lyric source + unobtrusive restore control */
(function(){
...
/* V0.22.68 BROADCAST READY — visible per-track subtitles + verified persistence */
